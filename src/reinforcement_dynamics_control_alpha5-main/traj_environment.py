...
diff = (3.14159265* 2) / 1000

trajectory = np.zeros([1000, 4])

for i in range(1000):
    trajectory[i,0] = 1.029 + math.sin(diff * i)
    trajectory[i,1] = 2.19 + math.sin(diff * i)
    trajectory[i,2] = 1.016 + math.sin(diff * i)
    trajectory[i,3] = 1.01 + math.sin(diff * i)
...
class armEnv(Env):
    ...
    def step(self, action):
        ...
        self.desired = trajectory[self.timestep - 1, 0:4]
        self.pid = PID_controller(self.desired)
        ...
        # check what happened
        _obs = self.robot._get_obs()
        error = trajectory[self.timestep - 1, 0:4] - _obs[0:4]
        ...
        reward = -(theta_error + (0.1 * action_change))
